...
pub fn service(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let mut _struct_fields = input.fields.clone();
    
    // Determine if we need to add fields based on struct type
    let _struct_has_fields = match &input.fields {
        ...
        quote! {
            impl #struct_name {
                ...
                fn register_actions(&mut self) {
                    // Create a mutable registry
                    let mut registry = std::collections::HashMap::new();
                    
                    // Call individual registration methods for each action
                    // These are auto-generated by the scanning for #[action] attributes
                    // Will be replaced during code generation with actual calls
                    
                    // Example:
                    // self.register_to_uppercase(&mut registry);
                    // self.register_concat(&mut registry);
                    
                    // Update the action registry
                    self.action_registry = std::sync::Arc::new(registry);
                }
                ...
                pub async fn register_subscriptions(&self, context: &runar_node::RequestContext) -> anyhow::Result<()> {
                    // Call individual subscription registration methods
                    // These are auto-generated by scanning for #[subscribe] attributes
                    // Will be replaced during code generation with actual calls
                    
                    // Example:
                    // self.register_handle_text_event_subscription(context).await?;
...
pub fn action(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        // Updated to take a mutable registry parameter for the auto-registration system
        #[inline]
        fn #register_fn_name(&self, registry: &mut std::collections::HashMap<String, ActionHandlerFn>) {
            ...
            // Register the handler in the provided registry
...
pub fn subscribe(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        // Register this subscription with the auto-registration system
